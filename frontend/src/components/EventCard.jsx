			...
			<article className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 w-full h-full p-6 m-0 border border-gray-200 hover:border-blue-300 hover:-translate-y-1">
				<section className="w-full flex justify-between mb-4">
					<p className="text-sm text-gray-600">
						Host:{" "}
						<strong className="text-blue-600 font-semibold">{event.host}</strong>
					</p>
					<p className="text-sm text-gray-600">
						Category:{" "}
						<strong className="text-teal-600 font-semibold">{event.category}</strong>
					</p>
				</section>
				...
				<section className="w-full flex justify-around pt-4 border-t border-gray-200">
					<section className="flex flex-col items-center">
						...
						<p className="text-sm font-semibold text-gray-700">
							{event.event_start ? formatDatetimeForUI(event.event_start) : "TBD"}
						</p>
					</section>
					<section className="flex flex-col items-center">
						<p className="m-0 mb-1 text-xs text-gray-500 font-medium">Event Finish</p>
						<p className="text-sm font-semibold text-gray-700">
							{event.event_end ? formatDatetimeForUI(event.event_end) : "TBD"}
						</p>
					</section>
				</section>
			</article>
